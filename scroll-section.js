...
if (!CSS.supports('(animation-timeline: scroll()) and (animation-range: 0% 100%)')) {
  ...
  const scroller = gsap.fromTo(document.documentElement, { '--hue': config.start }, { '--hue': config.end, ease: 'none' });
  ...
  chromaEntry = gsap.fromTo(document.documentElement, { '--chroma': 0 }, {
    '--chroma': 0.3,
    ease: 'none',
    scrollTrigger: {
      scrub: 0.2,
  ...
  chromaExit = gsap.fromTo(document.documentElement, { '--chroma': 0.3 }, {
    '--chroma': 0,
    ease: 'none',
    scrollTrigger: {
      scrub: 0.2,
